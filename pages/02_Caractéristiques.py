...
with st.container():
    ...
    fig = scatter_plot(df,feature,features[feature]["type"],comparison=features[feature]["comparison"],label_rot=features[feature]["label_rot"])
    st.pyplot(fig)

with st.container():
    st.subheader("Salaire moyen selon "+feature)
    fig = mean_plot(df,feature,features[feature]["type"],comparison=features[feature]["comparison"],label_rot=features[feature]["label_rot"])
    st.pyplot(fig)

with st.container():
    st.subheader("Salaire médian selon "+feature)
    fig = median_plot(df,feature,features[feature]["type"],comparison=features[feature]["comparison"],label_rot=features[feature]["label_rot"])
    st.pyplot(fig)
